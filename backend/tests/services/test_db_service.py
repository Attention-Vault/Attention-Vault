...
class TestDbService:
    ...
    async def test_store_contract_data_success(
        self, monkeypatch, test_db, sample_contract_data
    ):
        ...
        stored_contract = await test_db.contracts.find_one(
            {"contract_address": sample_contract_data["contract_address"]}
        )
        assert stored_contract is not None
        assert (
            stored_contract["contract_address"]
            == sample_contract_data["contract_address"]
        )
        assert (
            stored_contract["verification_text"]
            == sample_contract_data["verification_text"]
        )
        assert (
            stored_contract["twitter_handle"] == sample_contract_data["twitter_handle"]
        )
    ...
    async def test_update_contract_with_post_success(
        self, monkeypatch, test_db, sample_contract_data, sample_update_data
    ):
        ...
        result = await update_contract_with_post(
            sample_contract_data["contract_address"], sample_update_data
        )
    ...
    async def test_update_contract_with_post_not_found(
        self, monkeypatch, test_db, sample_update_data
    ):
        ...
        # Call the function with a non-existent contract address
        result = await update_contract_with_post(
            "non_existent_contract", sample_update_data
        )

        # Verify the result is False since no document was updated
        assert result is False

    async def test_update_contract_with_post_error(
        self, monkeypatch, sample_update_data
    ):
