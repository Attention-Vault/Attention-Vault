...
class MetricsRequest(BaseModel):
    ...
    start_date: Optional[str] = Field(None, description="Start date for metrics analysis (YYYY-MM-DD)")
    end_date: Optional[str] = Field(None, description="End date for metrics analysis (YYYY-MM-DD)")
...
class MetricsResponse(BaseModel):
    ...
    total_metrics: Dict[str, int] = Field(
        default_factory=lambda: {"impressions": 0, "likes": 0, "retweets": 0, "replies": 0, "quotes": 0}
    )