...
async def distribute_tranche(contract_address: str, tranche_index: int) -> bool:
    ...
    try:
        ...
        logger.info(
            f"Successfully distributed tranche {tranche_index} for contract {contract_address}"
        )
        return True

    except Exception as e:
        logger.error(
            f"Error distributing tranche {tranche_index} for contract {contract_address}: {str(e)}"
        )
        return False
