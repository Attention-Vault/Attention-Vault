...
export function ClusterChecker({ children }: { children: ReactNode }) {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.log("Connection successful for", cluster.name, ":", version);
        return version;
      } catch (error) {
        console.error("Connection failed for", cluster.name, "Error:", error);
        throw error;
      }
    },
    retry: 3,
  ...
  if (query.isLoading) {
    console.log("Validating connection for", cluster.name, "...");
    return null;
  }
...
export function ClusterUiSelect() {
  return (
    <div className="btn h-[36px] px-4 btn-primary rounded-lg text-sm font-semibold">
      Sonic SVM
    </div>
  );
}
