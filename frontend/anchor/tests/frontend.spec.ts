...
describe("frontend", () => {
  ...
  it("Initialize Frontend", async () => {
    ...
    const currentCount = await program.account.frontend.fetch(
  ...
  it("Increment Frontend", async () => {
    await program.methods
      .increment()
      .accounts({ frontend: frontendKeypair.publicKey })
      .rpc();
  ...
  it("Set close the frontend account", async () => {
    ...
    const userAccount = await program.account.frontend.fetchNullable(
      frontendKeypair.publicKey
    );
